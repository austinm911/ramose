...
This shape is a builder on the deploy side: `Ripple.claims(auth, { sub, db, class, attrs?, now? }, policy?)` (`alchemy/src/Auth.ts`) constructs the payload above from one `AuthConfig` (`{ issuer, audience, ttl }`) — the same value `authEnv({ auth })` pins `RIPPLE_JWT_ISS` / `RIPPLE_JWT_AUD` / `RIPPLE_JWT_MAX_TTL` from, so `exp - iat === ttl === maxTtl` by construction. It is pure (no signing, no I/O; the app signs with its own JWKS key) and validates at mint what the peer rejects at verify: the db name, and — given the compiled policy — that `class` is declared. For Better Auth apps, `@ripple/better-auth` (`packages/better-auth`) packages the whole mint route as a server plugin (`rippleToken({ auth, policy?, classOf })`, signing with the `jwt` plugin's JWKS) plus a client plugin whose `authClient.ripple.token({ db })` feeds `Ripple.token.jwt` directly.
...
**Handshake.** Under a policy a JWT is the *only* data-plane principal: `RIPPLE_TOKEN` grants no class on a `/db/:name` — it reaches `/health` and the no-op `ensure` case and nothing else, and an app tier acting for a user forwards that user's JWT rather than reusing it. Verify the JWT under an explicit per-verifier algorithm list. Then pin `iss`/`aud`, check `exp`, assert `ripple.db === db`, decode `Claims`, and resolve `principal` with one AVET lookup at the current basis. The frozen `Principal` is a **positional argument of `route()`**, so session sub-requests (`index.ts:127-137`, dispatch `:240`) cannot fall back to header sniffing. The per-isolate memo is keyed on `token + db`, ≤ 60 s.
