...
**Gate (done):** `Todo.owner.friends.name` types on TS 5.9.3 without `excessively deep`. Production ships
depth-capped / lazy self-ref nav and `attrName` (not `.name`) so paths are not shadowed. A first usable client
slice is in: `Ripple.query` / targeted `Ref` / find-pull lowering / `db.q`·`live` on query values; `.orderBy` /
`.limit` are still client-side until core AST grows them.
