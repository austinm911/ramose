 ...
 */
export function useTransact(options?: {
  onError?: (error: unknown) => void;
}): Transact;
export function useTransact(
  ...rest: [options?: { onError?: (error: unknown) => void }, slot?: symbol]
): Transact {
  const [args, slot] = splitSlot(rest);
  const options = args[0] as { onError?: (error: unknown) => void } | undefined;

  const [inFlight, setInFlight] = useState(0, subSlot(slot, "transact:in-flight"));
  const [error, setError] = useState<unknown | undefined>(
    undefined,
    subSlot(slot, "transact:error"),
  );
