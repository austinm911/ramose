...
> {
  ...
  const pendingFor = useCallback(
    ...
    [pendingKeys],
    subSlot(slot, "op:pending-for"),
  );
  const errorFor = useCallback(
    (key: unknown) => errors.get(key),
    [errors],
    subSlot(slot, "op:error-for"),
  );
