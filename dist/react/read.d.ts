...
export type SuspendedRead<A, E = DbError> = Read<A, E> & {
    readonly data: A;
};
...
export declare const READ_INITIAL: ReadState<never, never>;
export declare const asLoading: <A, E>(prev: ReadState<A, E>) => ReadState<A, E>;
export declare const asSuccess: <A, E>(data: A, t: number | undefined) => ReadState<A, E>;
export declare const asError: <A, E>(prev: ReadState<A, E>, error: E) => ReadState<A, E>;
/** First-paint state: hydrated rows, or the empty loading shell. */
export declare const hydrateRead: <A, E>(options?: ReadOptions<A>) => ReadState<A, E>;
/**
 * The basis a view reads at, without `GET /info`: pinned `asOf(t)`, else
 * `session.t` once the session has seen a frame. `0` is "not yet".
 */
export declare const readT: <C extends Schema.Any>(db: ReadDb<C> | undefined) => number | undefined;
