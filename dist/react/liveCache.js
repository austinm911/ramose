...
import { shareEqualDeep } from "../db/shareEqualDeep.js";
const cache = new Map();
 ...
 */
export const retainLive = (key, create, finalize) => {
    ...
    let last = NONE;
    return {
        subscribe: (onValue, onError) => held.sub.subscribe((raw) => {
            ...
            if (last !== NONE)
                rows = shareEqualDeep(last, rows);
            last = rows;
            onValue(rows);
        }, (error) => {
            if (cache.get(key) === held)
                cache.delete(key);
            onError?.(error);
        }),
