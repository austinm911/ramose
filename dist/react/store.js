import { PENDING, toQueryState } from "./query-state.js";
/**
...
class QueryStore {
    ...
    listeners = 0;
    hold;
    constructor(database, key, source) {
    ...
    retain = (onChange) => {
        ...
        return () => {
            ...
            this.listeners--;
            if (this.listeners === 0)
                release(this.database, this.key, this.erased());
        };
    };
    /**
...
const adopt = (database, key, store) => {
 ...
 */
const evictUnclaimed = (stores, room) => {
    if (stores.unclaimed.size <= room)
        return;
    for (const key of stores.unclaimed) {
        ...
        if (hold !== undefined && !hold.settled())
            continue;
 ...
 */
export const reviewUnclaimed = (database) => {
    const stores = STORES.get(database);
    if (stores !== undefined)
        evictUnclaimed(stores, UNCLAIMED_LIMIT);
};
const release = (database, key, store) => {
    const stores = STORES.get(database);
    if (stores?.stores.get(key) !== store)
        return;
    stores.stores.delete(key);
    stores.unclaimed.delete(key);
};
export const queryStore = (database, key, observe) => {
 ...
 */
export const heldStoreCount = (database) => STORES.get(database)?.stores.size ?? 0;
