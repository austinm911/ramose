"use client";
import { peekClaims } from "../db/token.js";
import { useContext, useEffect, useMemo, useState } from "react";
import { RamoseContext } from "./context.js";
const missingProvider = (hook) => new Error(`${hook}: no <RamoseProvider> above this component. ` +
 ...
 */
const useRamoseContext = () => {
    const ctx = useContext(RamoseContext);
    if (ctx === null)
        throw missingProvider("useDb");
    return ctx;
};
/**
 ...
 * state to render around.
 */
const useClient = () => useRamoseContext().client;
/**
 ...
 * worthless.
 */
export const useDb = (name, schema) => {
 ...
 */
export const useRamoseClaims = () => {
    const ctx = useContext(RamoseContext);
    if (ctx === null)
        throw missingProvider("useRamoseClaims");
    const { token } = ctx;
