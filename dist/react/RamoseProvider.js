...
export const RamoseProvider = (props) => {
    const { url, token, fetch, webSocket, children } = props;
    const [generation, reconnect] = useReducer((n) => n + 1, 0);
    ...
    const client = useMemo(() => connect({ url, token, fetch, webSocket }), [url, token, fetch, webSocket, generation]);
    useEffect(() => {
    ...
    return (_jsx(RamoseContext.Provider, { value: { client, token }, children: children }));
};
