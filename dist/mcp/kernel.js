import { fieldKey } from "../internal/authorization/validation/common.js";
import { operationGrantAllows, principalValuesEqual, projectPrincipalTerm, } from "../internal/authorization/operation-grant.js";
import { runOneShotRead } from "../internal/authorization/reads.js";
import { QueryBudgetError } from "../internal/core/query/engine.js";
import { Index, ValueTag } from "../internal/core/datom.js";
import { toJson } from "../internal/core/json.js";
import { RAMOSE_TYPE_IDENT } from "../internal/core/schema.js";
import { DEFAULT_QUERY_LIMIT, MAX_DESCRIBE_ITEMS, MAX_SELECT_FIELDS, encodeOperationVersionToken, McpToolFailure, toolFailure, } from "./contract.js";
const GRAPH_TRAIT_IDENT = ":graph";
const GRAPH_NAME_IDENT = ":graph/name";
const visibleGraphNames = async (context, limit) => {
    ...
    const found = new Set();
    let truncated = false;
    outer: for await (const chunk of context.filteredDb.datoms(Index.AEVT, { a: attribute.id })) {
        for (const datom of chunk) {
            ...
            if (typeof type !== "string" || !composition.isEntityIdent(type))
                continue;
            if (!composition.transitiveTraits(type).includes(GRAPH_TRAIT_IDENT))
                continue;
            if (found.has(datom.v))
                continue;
            if (found.size >= limit) {
                truncated = true;
                break outer;
...
export const describeGraph = async (context, caller, at) => {
    ...
    for (const descriptor of catalog.operations) {
        if (!operationGrantAllows(context.unit, descriptor, caller, context.principal.subject))
            continue;
...
const composedTraits = (catalog, entity) => {
    const names = new Set();
    for (const composition of catalog.traitComposition) {
        if (composition.composer.name !== entity)
            continue;
        names.add(composition.trait.name);
        for (const transitive of composition.transitive)
            names.add(transitive.name);
    }
    return names;
};
const staticRuleTruth = (expr, caller, subject) => {
    const project = (term) => projectPrincipalTerm(term, caller, subject);
    switch (expr._tag) {
        case "const":
            return expr.value;
        case "hasClass":
...
const decisionStaticallyDenies = (decision, rules, caller, subject) => {
    for (const id of decision.deny) {
        const rule = rules.get(id);
        if (rule === undefined)
            return true;
        if (staticRuleTruth(rule.expr, caller, subject) === true)
            return true;
    }
    for (const id of decision.allow) {
        ...
        if (rule === undefined)
            continue;
        if (staticRuleTruth(rule.expr, caller, subject) !== false)
            return false;
    }
    return true;
};
const staticallyHiddenEntity = (context, caller, entity) => {
    const policy = context.unit.policy;
    const decision = policy.decisions.entities.find((entry) => entry.target.name === entity)?.decision;
    if (decision === undefined)
        return true;
    return decisionStaticallyDenies(decision, new Map(policy.rules.map((rule) => [rule.id, rule])), caller, context.principal.subject);
};
const staticallyHidden = (context, caller, field) => {
    const policy = context.unit.policy;
    ...
    if (field.id.owner.kind === "trait") {
        const traitDecision = policy.decisions.traits.find((entry) => entry.target.name === field.id.owner.name)?.decision;
        if (traitDecision === undefined)
            return true;
        if (decisionStaticallyDenies(traitDecision, rules, caller, subject))
            return true;
    }
    const decision = policy.decisions.fields.find((entry) => fieldKey(entry.target) === fieldKey(field.id))?.decision;
    if (decision === undefined)
        return false;
    return decisionStaticallyDenies(decision, rules, caller, subject);
};
const scalarFieldsOf = (context, caller, entity) => {
    const catalog = context.unit.catalog;
    ...
    for (const field of catalog.fields) {
        ...
        if (context.currentDb.attr(ident) === undefined)
            continue;
        if (staticallyHidden(context, caller, field))
            continue;
...
export const requireBoundedImplicitProjection = (visibleFields) => {
    if (visibleFields > MAX_SELECT_FIELDS) {
        throw toolFailure("invalid_query", `this entity exposes more than ${MAX_SELECT_FIELDS} readable fields; ` +
            "name the ones you want with an explicit select");
    }
};
export const lowerQueryDocument = (context, caller, document) => {
    const entity = context.unit.catalog.entities.find((candidate) => candidate.id.name === document.from.entity);
    if (entity === undefined ||
    ...
    if (document.select === undefined)
        requireBoundedImplicitProjection(available.size);
    ...
    if (selected.length === 0)
        return undefined;
    const inputs = [`:${entity.id.name}`];
    const inSpec = ["$", "?type"];
    const where = [["?e", RAMOSE_TYPE_IDENT, "?type"]];
    let index = 0;
...
export const queryReadFailure = (cause) => cause instanceof QueryBudgetError
    ? toolFailure("query_budget_exceeded", "the query exceeded its read budget; narrow it with more specific " +
        "filters or a smaller limit")
    : undefined;
export const runQueryDocument = async (context, caller, document, options = {}) => {
    const lowered = lowerQueryDocument(context, caller, document);
    if (lowered === undefined)
        return Object.freeze({ rows: [], truncated: false });
    let result;
    try {
        result = await runOneShotRead(context.filteredDb, { kind: "query", query: lowered.query, inputs: lowered.inputs }, options.maxCells === undefined ? {} : { maxCells: options.maxCells });
    }
    catch (cause) {
    ...
    for (const row of pulled) {
        ...
        for (const field of lowered.fields) {
            if (record[field.ident] !== undefined) {
                projected.push([field.name, toJson(record[field.ident])]);
            }
        }
        if (projected.length > 0)
            rows.push(Object.fromEntries(projected));
    }
    if (rows.length === 0)
        return Object.freeze({ rows: [], truncated: false });
    return Object.freeze({
...
export const publishableWithoutReferences = (shape) => {
    switch (shape._tag) {
        case "scalar":
            return true;
        case "array":
        ...
        case "struct":
            return shape.fields.every((field) => publishableWithoutReferences(field.shape));
        case "ref":
        case "opaque":
            return false;
    }
};
export const projectOperationOutcome = (shape, output) => publishableWithoutReferences(shape) ? output : OUTCOME_WITHHELD;
export const publicMutateResult = (result, outputShape) => {
    switch (result._tag) {
        case "Completed":
            return Object.freeze({
        ...
        case "Conflict":
            return toolFailure("invocation_conflict", "that invocationId already names a different invocation").envelope;
        case "OperationChanged":
            return toolFailure("operation_changed", "the pinned operation version is not the deployed one; rediscover it").envelope;
        case "UpdateRequired":
            return toolFailure("invocation_update_required", "this invocation must be retried under a fresh invocationId").envelope;
        case "Failed":
            return toolFailure("internal_error", "the operation did not complete").envelope;
        case "Indeterminate":
            return toolFailure("invocation_indeterminate", "the outcome is not yet decidable; retry the same invocationId").envelope;
        case "Rejected":
