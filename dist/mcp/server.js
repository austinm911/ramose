...
import { MAX_QUERY_LIMIT, McpToolFailure, errorEnvelope, } from "./contract.js";
const EXPERIMENTAL = "EXPERIMENTAL: Ramose's MCP surface makes no compatibility promise yet. " +
...
const AT_SCHEMA = {
    ...
    maxItems: 16,
    description: "Graph names from your authorized root. [] selects the root itself.",
};
const TOOLS = Object.freeze([
    {
        name: "describe",
        title: "Describe visible capabilities",
        description: "List the entity names, invocable operations, and child graph names visible " +
    ...
    },
    {
        ...
        description: "Read rows of one entity. The query document is { version: 1, from: " +
    ...
    },
    {
        ...
        description: "Invoke exactly one operation describe listed, pinned to the version it " +
...
export const handleMcpRequest = async (request, tools) => {
    const server = new Server({ name: "ramose", title: "Ramose (experimental)", version: "experimental" }, {
        capabilities: { tools: {} },
    ...
    server.setRequestHandler(CallToolRequestSchema, async (call) => {
        const body = tools[call.params.name];
        ...
        try {
            ...
            if (cause instanceof McpToolFailure)
                return failedResult(cause.envelope);
