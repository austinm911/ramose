 ...
 * created offline.
 */
export type MutationInput<S> = S extends RamoseVt<"ref"> ? RefInput<S> : S extends {
    readonly fields: infer Fields;
} ? StructInput<S, Fields> : S extends {
    readonly value: infer Item;
} ? CodecType<S> extends readonly unknown[] ? readonly MutationInput<Item>[] : CodecType<S> : S extends {
    readonly schema: infer Inner;
} ? MutationInput<Inner> : S extends {
    readonly to: infer Decoded;
} ? MutationInput<Decoded> : CodecType<S>;
type RefInput<S> = S extends {
    readonly _target?: infer Target;
} ? [NonNullable<Target>] extends [AnyEntity] ? MutationRef<NonNullable<Target>> : MutationRef : MutationRef;
type StructInput<S, Fields> = {
    readonly [K in keyof CodecType<S>]: K extends keyof Fields ? MutationInput<Fields[K]> : CodecType<S>[K];
};
/**
...
type MutationCall<In> = {} extends In ? (input?: In) => Receipt : (input: In) => Receipt;
type MethodFor<Op> = Op extends {
    readonly input: infer In;
} ? MutationCall<MutationInput<In>> : MutationMethod;
type OperationsOf<Owner> = Owner extends {
    readonly [OwnedOperations]: infer Ops extends object;
} ? Ops : never;
type SelfFlag<Op> = Op extends {
    readonly self: infer Flag;
} ? Flag : never;
type PlacementKnown<Op> = boolean extends SelfFlag<Op> ? false : true;
type MethodsWhereSelf<Owner, Self extends boolean> = {
    readonly [K in keyof OperationsOf<Owner> as PlacementKnown<OperationsOf<Owner>[K]> extends false ? K : OperationsOf<Owner>[K] extends {
        readonly self: Self;
    } ? K : never]: PlacementKnown<OperationsOf<Owner>[K]> extends false ? MutationMethod : MethodFor<OperationsOf<Owner>[K]>;
};
type TraitsOf<Owner> = Owner extends {
    readonly traits: infer Traits extends readonly unknown[];
} ? Traits[number] : never;
type TraitClosure<T> = T extends {
    readonly _tag: "Trait";
} ? T | TraitClosure<TraitsOf<T>> : never;
type Namespace<Owners, Self extends boolean> = Simplify<UnionToIntersection<Owners extends unknown ? MethodsWhereSelf<Owners, Self> : never>>;
type SchemaOwners<S extends AnySchema> = S["entities"][keyof S["entities"]] | TraitClosure<TraitsOf<S["entities"][keyof S["entities"]]>>;
type NamesEntities<S extends AnySchema> = string extends keyof S["entities"] ? false : true;
type NamesFields<N extends AnyComposer> = string extends keyof N["fields"] ? false : true;
/** The targetless operations one catalog declares. */
export type DatabaseMutations<S extends AnySchema> = NamesEntities<S> extends true ? Namespace<SchemaOwners<S>, false> : MutationNamespace;
/**
 ...
 */
export type EntityMutations<N extends AnyComposer> = N extends AnyComposer ? NamesFields<N> extends true ? Namespace<N | ComposedTraits<N>, true> : MutationNamespace : never;
type ComposedTraits<N> = N extends {
    readonly _tag: "Entity";
} ? TraitClosure<TraitsOf<N>> : never;
export {};
//# sourceMappingURL=mutation-schema.d.ts.map