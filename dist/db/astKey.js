...
import { inspectPullField, isAgain, isAllShape, lowerPullPattern, } from "./Pull.js";
import { lowerQueryObject } from "./query/index.js";
const astKeyMemo = new WeakMap();
const pullKeyMemo = new WeakMap();
...
export const canonicalAstKey = (ast) => JSON.stringify(sortKeys(toJson(ast)));
const ERROR_PREFIX = "\0error:";
/** Always compute a key — used by the purity guard. */
export const computeAstKey = (query) => {
    try {
 ...
 * (small ASTs).
 */
export const queryAstKey = (query) => {
 ...
 */
export const queryStructureKey = (query) => queryAstKey(query);
/**
 ...
 */
const withOptionalMarkers = (pattern) => {
...
export const computePullPatternKey = (pattern) => {
    try {
 ...
 * lowers again (small).
 */
export const pullPatternKey = (pattern) => {
    if (typeof pattern === "object" && pattern !== null) {
 ...
 */
export const liveSubscriptionKey = (viewKey, query) => `${viewKey}\0${queryAstKey(query)}`;
const PURITY_WARNING = "ramose/react: query body is not pure — two lowerings produced different " +
 ...
 * is that footgun.
 */
export const assertLoweringPurity = (query) => {
...
//# sourceMappingURL=astKey.js.map