...
import { inspectPullField } from "../Pull.js";
import { isCursor, isPipeline, lowerQueryObject, symbolicIdentityLowering, } from "./query.js";
const kindOfValueType = (vt) => {
    switch (vt) {
        case "instant":
            return "instant";
        case "bytes":
...
const kindOfKey = (key, pipe) => {
    if (typeof key === "object" && key !== null && "valueType" in key) {
        return kindOfValueType(key.valueType);
    }
    if (typeof key === "string" && pipe !== undefined) {
        ...
        if (select !== undefined && select.kind === "select") {
            const field = select.shape[key];
            if (field !== undefined) {
                const attr = inspectPullField(field).attr;
                if (attr?.valueType !== undefined)
                    return kindOfValueType(attr.valueType);
            }
        }
        const field = pipe.ns.fields[key];
        if (field?.valueType !== undefined)
            return kindOfValueType(field.valueType);
        if (key === "id")
            return "number";
    }
    return "unknown";
};
const pagedQuery = (q) => q.seek !== undefined ? q : q.after(null);
const cursorKeyCount = (q) => {
    const order = lowerQueryObject(pagedQuery(q), symbolicIdentityLowering().lowering).query.order;
    if (order === undefined || order.length === 0) {
        throw new Error("ramose/query: encodeCursor / decodeCursor pages a sorted query — add an orderBy for the cursor to be a position in");
    }
    return order.length;
};
const cursorKeyKinds = (q) => {
    ...
    if (pipe !== undefined) {
        for (const st of pipe.stages) {
            if (st.kind === "orderBy")
                kinds.push(kindOfKey(st.key, pipe));
        }
    }
    for (const o of q.orders)
        kinds.push(kindOfKey(o.key, pipe));
    while (kinds.length < n)
        kinds.push("number");
    return kinds.slice(0, n);
};
const retypeKey = (value, kind) => {
    switch (kind) {
        case "instant": {
 ...
 */
export const encodeCursor = (q, cursor) => {
    if (!isCursor(cursor)) {
        throw new Error("ramose/query: encodeCursor takes a Cursor");
    }
    const n = cursorKeyCount(q);
    if (cursor.keys.length !== n) {
        throw new Error(`ramose/query: this cursor does not fit — it carries ${cursor.keys.length} sort-key values and the query orders by ${n}; a cursor only continues the query that minted it`);
    }
 ...
 */
export const decodeCursor = (q, encoded) => {
    ...
    const n = cursorKeyCount(q);
    if (raw.length !== n) {
        throw new Error(`ramose/query: this cursor does not fit — it carries ${raw.length} sort-key values and the query orders by ${n}; a cursor only continues the query that minted it`);
    }
