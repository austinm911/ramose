...
}[keyof N["fields"]] | ":db/id";
export type AttrIdent<A> = A extends {
    readonly ident: infer I;
} ? I : never;
...
type FocusId<N extends AnyComposer> = N extends {
...
} ? E extends AnyEntity ? E : AnyEntity : AnyEntity : A extends {
    readonly ident: `:${infer Ns}/${string}`;
} ? Entity<Ns, FieldMap> : AnyEntity;
...
} ? Exclude<T, undefined> extends AnyComposer ? Exclude<T, undefined> : Enclosing : Enclosing;
type OwnerNs<A> = A extends {
    readonly ident: `:${infer Ns}/${string}`;
} ? Ns : never;
export type ReverseOk<A, N extends AnyComposer> = [RefTarget<A, AnyComposer>] extends [
    AnyComposer
] ? [AnyComposer] extends [RefTarget<A, AnyComposer>] ? [OwnerNs<A>] extends [N["ns"]] ? true : false : [RefTarget<A, N>] extends [N] ? true : false : [RefTarget<A, N>] extends [N] ? true : false;
export {};
