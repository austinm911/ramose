...
import { TX_BASE } from "../../internal/core/schema.js";
...
import { inspectPullField, isAgain, isAllShape, lowerPullPattern, reshapePullResult, } from "../Pull.js";
 ...
 */
export const tryLowerQueryObject = (qv) => {
    try {
...
export const lowerQueryObject = (qv) => {
    resetGensym();
    const ctx = { clauses: [] };
    const built = runInto(qv, ctx, qv.stripCursor);
    const names = new Map();
    ...
    const registerMembership = (ns) => {
        ...
        byNs.set(ns, entry);
        const e = freshName("m");
        const idents = Object.values(ns.fields).map((a) => a.ident);
    ...
    // ── positions ────────────────────────────────────────────────────────────
    const lowerConst = (v, _use) => unwrapEidLike(v);
    /** e/v/tx/op position of a fact: a var name, a constant, or "_". */
    const lowerPos = (v, use) => {
    ...
    const neverClause = () => [["ground", []], [freshName("n"), "..."]];
    // ── clauses ──────────────────────────────────────────────────────────────
    ...
    const readVar = (v) => {
        switch (v.kind) {
            ...
            case "tx":
                ...
                return (cell) => typeof cell === "number" ? { id: makeEid(cell) } : cell;
            case "t":
    ...
    const flattenCell = (path, cell) => {
        ...
        if (isAggSpec(cell)) {
            const v = isFocusSentinel(cell.v)
                ...
                : cell.v;
            const read = v.kind === "t" && (cell.fn === "min" || cell.fn === "max")
                ? (x) => (typeof x === "number" ? x - TX_BASE : x)
                : (x) => x;
    ...
    if (isValueSpec(proj)) {
        ...
        finalizeRows = (tuples) => tuples.map((t) => typeof t[0] === "number" ? { id: makeEid(t[0]) } : t[0]);
    }
    ...
    const bindOrderPath = (path, revs, dir, empty) => {
