...
import { MAX_PRODUCED_TEXT_UNITS, asciiLower, asciiUpper, canonicalKey, clampIndex, codePoints, deepEquals, isTimestamp, trimPinned, } from "./values.js";
const bool = (args, index) => args[index];
const num = (args, index) => args[index];
const txt = (args, index) => args[index];
const coll = (args, index) => args[index];
const roundHalfAwayFromZero = (value) => value < 0 ? -Math.round(-value) : Math.round(value);
const codePointIndexOf = (value, needle) => {
    if (needle.length === 0)
        return 0;
...
const occurrences = (value, search) => {
    let count = 0;
    let from = 0;
    for (;;) {
...
export const standardLibraryImplementationsV1 = {
    ...
    "text.concat": (args) => {
        ...
        if (left.length + right.length > MAX_PRODUCED_TEXT_UNITS)
    ...
    "text.equalsIgnoreCase": (args) => asciiLower(txt(args, 0)) === asciiLower(txt(args, 1)),
    "text.compare": (args) => {
        const a = txt(args, 0);
        const b = txt(args, 1);
        if (a < b)
    ...
    "text.replace": (args) => {
        ...
        const produced = value.length + occurrences(value, search) * (replacement.length - search.length);
        if (produced > MAX_PRODUCED_TEXT_UNITS)
            return OUTPUT_TOO_LARGE;
    ...
    "text.join": (args) => {
        ...
        if (parts.length > 1)
            produced += (parts.length - 1) * separator.length;
        if (produced > MAX_PRODUCED_TEXT_UNITS)
            return OUTPUT_TOO_LARGE;
