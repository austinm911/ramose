 ...
 */
export declare const EntityId: typeof Schema.Number;
/**
...
declare const PrefixHalt_base: new <A extends Record<string, any> = {}>(args: import("effect/Types").VoidIfEmpty<{ readonly [P in keyof A as P extends "_tag" ? never : P]: A[P]; }>) => import("effect/Cause").YieldableError & {
    readonly _tag: "ramose/PrefixHalt";
} & Readonly<A>;
/**
 ...
 */
export declare class PrefixHalt extends PrefixHalt_base<{}> {
}
...
export interface RuntimeOp {
    ...
    effect<A, E = never>(name: string, run: (ctx: {
        readonly env: unknown;
        readonly principal: OpPrincipal;
        readonly databases: {
            install(schema: AnySchema, name?: string): Effect.Effect<unknown, DbError>;
        };
    }) => Effect.Effect<A, E> | Promise<A>): Effect.Effect<A, E | DbError>;
}
...
declare const defineOperation: <Name extends string, I, O = {}, C extends AnySchema = AnySchema, N extends OnEntity<C> = undefined>(name: Name, schemas: OperationSchemas<I, O, N, C>, body: (op: Op<C, N>, input: I) => Promise<OutputDraft<O>> | OutputDraft<O>) => Operation<Name, I, O, N, C>;
...
export interface OpReport<O = unknown, C extends AnySchema = AnySchema> extends TxReport<C> {
    readonly output: O;
}
 ...
 */
export declare const materializeOutput: (value: unknown, tempids: Readonly<Record<string, number>>) => unknown;
 ...
 */
export declare const finalizeOutput: (schema: Schema.Codec<unknown, unknown>, value: unknown, tempids: Readonly<Record<string, number>>) => Effect.Effect<unknown, InvalidRequest>;
