...
export { IncompatibleSchema, PolicyError, } from "./SchemaErrors.js";
 ...
 */
export class QueryBudgetExceeded extends Data.TaggedError("QueryBudgetExceeded") {
}
 ...
 */
export class OperationsCoverageError extends Data.TaggedError("OperationsCoverageError") {
}
const TAGS = new Set([
    "TxRejected",
    "Unavailable",
...
export const isDatabaseError = (value) => typeof value === "object" &&
    value !== null &&
    TAGS.has(value._tag ?? "");
...
const opt = (key, value) => typeof value === "string" && value.length > 0 ? { [key]: value } : {};
/**
...
export const fromResponse = (status, body, headers) => {
    ...
    switch (b.tag) {
        ...
        case "TransactorDead": {
            ...
            const retryAfterMs = num(b.retryAfterMs, header === null || header === undefined ? 0 : Number(header) * 1000);
...
const isCloudflarePlatform = (message) => /<!DOCTYPE html>|Page not found|There is nothing here yet|error code:\s*1\d{3}/i.test(message);
