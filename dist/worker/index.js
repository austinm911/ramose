 ...
 */
import { DEFAULT_QUERY_MAX_CELLS, Histogram, RateMeter, allows, componentLogger, fromJson, isSuperuser, normalizePullPattern, pull, query, setTelemetryLevel, toJson } from "../internal/core/index.js";
import { envInt, internalHeaders } from "../internal/transactor/index.js";
import { TransactorDO } from "../internal/transactor/transactor-do.js";
import { QueryReplicaDO } from "../internal/replica/index.js";
import * as Effect from "effect/Effect";
import { Analytics, bindingOf, fromBinding, httpPoint, routeOf } from "./analytics.js";
import { allowedOrigin, allowsRawTransact, authState, cachedProvision, checkWrite, describePrincipal, isTokenOnly, principalOf, rememberProvisioned, shouldProvision, viewDb } from "./auth.js";
import { isDatabaseName } from "../db/DatabaseName.js";
import { BadRequest, NotFound, OperationRejected, Unauthorized, UpstreamError, fromThrown, toHttp } from "./errors.js";
import { operationNames } from "../db/Operation.js";
import { prepareOperation } from "./operations.js";
import { basisHeaders, coloHeader, fetchBasisWithStats, hintOf, invalidateBasis, nearestReplica, regionOf, replicaId, segmentSource } from "./peer.js";
import { PRINCIPAL_HEADER } from "./session.js";
import { WRITES_HEADER, isUnrecognizedWrites, resolveWrites } from "../writes.js";
export { resolveWrites } from "../writes.js";
...
export { toHttp, fromThrown, isRamoseError } from "./errors.js";
...
const writesWarned = new Set();
const unrecognizedWritesWarned = new Set();
/** Test hook: forget the policy + writes: "all" / unrecognized-env warnings. */
export const clearWritesWarning = () => {
    writesWarned.clear();
...
function warnWritesAll(env, writes) {
    if (writes !== "all" || authState(env).policy === undefined)
        return;
    const key = env.RAMOSE_POLICY ?? "";
    if (writesWarned.has(key))
        return;
    writesWarned.add(key);
    plog.warn(WRITES_ALL_POLICY_EVENT, {
        message: 'writes is "all" while a policy is installed — "all" only opens raw /transact when no policy is configured',
    });
}
function warnUnrecognizedWrites(envWrites) {
    if (!isUnrecognizedWrites(envWrites))
        return;
 ...
 */
function unknownPullAttrs(db, pattern, seen = []) {
    for (const spec of pattern) {
        if (spec.kind !== "attr")
            continue;
        if (spec.attr !== ":db/id")
            noteUnknown(db, spec.attr, seen);
        // a nested collection's :where / :order walk paths of their own
        for (const p of spec.where ?? [])
            unknownElemPredAttrs(db, p, seen);
        for (const o of spec.order ?? [])
            unknownPathAttrs(db, o.path, seen);
        if (spec.sub !== undefined)
            unknownPullAttrs(db, spec.sub, seen);
    }
    return seen;
}
function noteUnknown(db, ident, seen) {
    if (db.attr(ident) === undefined && !seen.includes(ident))
        seen.push(ident);
}
function unknownPathAttrs(db, path, seen) {
    for (const ident of path)
        if (ident !== ":db/id")
            noteUnknown(db, ident, seen);
}
function unknownElemPredAttrs(db, pred, seen) {
    if ("and" in pred)
        ...
        unknownElemPredAttrs(db, pred.not, seen);
        ...
        unknownPathAttrs(db, pred.path, seen);
}
...
function withCors(env, request, res) {
    ...
    if (origin === undefined || res.status === 101)
        return res;
...
function respond(err) {
    const h = toHttp(err);
    if (h.raw !== undefined)
        return new Response(h.raw, { status: h.status, headers: h.headers ?? { "content-type": "application/json", ...CORS } });
...
const recover = (info, t0) => ({
    NotFound: (e) => Effect.sync(() => respond(e)),
    BadRequest: (e) => Effect.sync(() => respond(e)),
    Unauthorized: (e) => Effect.sync(() => respond(e)),
    UpstreamError: (e) => Effect.sync(() => respond(e)),
    ...
    OperationRejected: (e) => Effect.sync(() => respond(e)),
});
...
const recordHttp = (request, info, status, ms) => Effect.gen(function* () {
    ...
    if (!ae.bound)
        return;
    yield* ae.writeDataPoint(httpPoint({ db: info.db, colo: request.cf?.colo, route: info.route, status, ms }));
 ...
 */
async function ingress(request, env, db, principal, text, t0) {
    let raw;
    try {
        raw = JSON.parse(text);
    }
    ...
    try {
        ...
        if (checked.kind === "skip")
            return { done: json({ t: bf.basis.t, txEid: 0, tempids: {}, datoms: [], ...(clientTxId !== undefined ? { clientTxId } : {}) }, 200, { "x-ramose-ms": String(Date.now() - t0) }) };
 ...
 */
async function withProvisioned(env, principal, db, transactor, txUrl, request) {
    if (authState(env).policy === undefined || !shouldProvision(principal))
        return principal;
    const hit = cachedProvision(principal);
    if (hit !== undefined)
        return { ...principal, eid: hit };
    try {
...
async function route(request, env, url, db, rest, principal, t0, peer) {
    ...
    const adminOnly = () => {
        if (policy !== undefined && !isSuperuser(principal, policy))
            throw new Unauthorized({ status: 403, message: "superuser only", code: "policy" });
    };
    ...
    if (rest === "/op" && request.method === "POST") {
        ...
        if (opName.length === 0)
            throw new BadRequest({ message: "body must be { name, input, clientOpId? }" });
        const clientOpId = typeof raw.clientOpId === "string" && raw.clientOpId.length > 0 ? raw.clientOpId : undefined;
        if (clientOpId !== undefined) {
            ...
            if (replay.ok) {
                const hit = fromJson(await replay.json());
        ...
        if (!res.ok)
            throw new UpstreamError({ status: res.status, body: await res.text(), headers });
        const ack = fromJson(await res.json());
    ...
    if (rest === "/query" && request.method === "POST") {
        ...
        if (!body?.query)
            throw new BadRequest({ message: "body must be { query, inputs? }" });
        if (body.explain)
            adminOnly(); // planner metadata is not filtered
        const bf = await fetchBasisWithStats(env, db, request);
        ...
        const result = await query(dbv, body.query, body.inputs ?? [], { stats, maxCells: envInt(env.RAMOSE_QUERY_MAX_CELLS, DEFAULT_QUERY_MAX_CELLS) });
        ...
        return json({ t: basis.t, root: basis.root.t, result, ...(body.explain ? { explain: stats.clauses, budget: stats.budget } : {}) }, 200, { "x-ramose-ms": String(Date.now() - t0), "x-ramose-r2-gets": String(after.r2Gets - before.r2Gets), "x-ramose-cache-hits": String(after.cacheHits - before.cacheHits), ...basisHeaders(request, env, bf) });
    }
    if (rest === "/pull" && request.method === "POST") {
        ...
        if (unknown.length > 0)
            throw new BadRequest({ message: `unknown attribute${unknown.length > 1 ? "s" : ""} in pull pattern: ${unknown.join(", ")}` });
        const eid = typeof body.eid === "number" ? body.eid : await dbv.entid(body.eid);
        if (eid === undefined)
            return json({ t: basis.t, result: null }, 200, { "x-ramose-ms": String(Date.now() - t0), ...basisHeaders(request, env, bf) });
    ...
    if (rest === "/info" && request.method === "GET") {
        ...
        const doJson = async (res) => {
            const body = await res.text();
            if (!res.ok)
                throw new UpstreamError({ status: res.status, body });
    ...
    if (rest === "/session" && request.method === "GET") {
        if ((request.headers.get("Upgrade") ?? "").toLowerCase() !== "websocket")
            throw new BadRequest({ message: "expected websocket" });
        ...
        if (hint)
            headers.set("x-ramose-replica-hint", hint);
        ...
        plog.debug("session.open", { db, colo: request.cf?.colo });
...
const handle = (request, env, t0, info, peer) => Effect.gen(function* () {
    if (!levelApplied) {
        levelApplied = true;
    ...
    if (!validDbName(db))
        return yield* Effect.fail(new BadRequest({ message: "invalid database name" }));
...
const runFetch = (request, env, peer) => {
    const t0 = Date.now();
    const info = { db: "-", path: "-", route: "other" };
    return Effect.runPromise(handle(request, env, t0, info, peer).pipe(Effect.catchTags(recover(info, t0)), Effect.map((res) => withCors(env, request, res)), Effect.tap((res) => recordHttp(request, info, res.status, Date.now() - t0)), Effect.provideService(Analytics, fromBinding(bindingOf(env)))));
};
/**
