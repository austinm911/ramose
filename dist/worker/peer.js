...
export function nearestReplica(env, db, request) {
...
export function wantsBasisCache(request, env) {
    const h = request.headers.get("x-ramose-cache-basis") ?? env?.RAMOSE_CACHE_BASIS ?? "1";
    return h !== "0";
}
export function cacheModeOf(request, env) {
...
export function minTOf(request) {
...
export async function fetchBasisWithStats(env, db, request) {
    ...
    if (useCache) {
        ...
        if (!hit)
            reason = "miss";
        else if (Date.now() - hit.at >= ttl)
            reason = "expired";
        else if (minT !== undefined && hit.basis.t < minT)
            reason = "min-t";
        else
            return { basis: hit.basis, hit: true, reason: "hit", calls: 0, behind: false };
    }
