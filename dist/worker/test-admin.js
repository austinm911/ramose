...
import { makeEntityIdScope, openEntityId, sealEntityId, sealingKeyOf, } from "../internal/replication/index.js";
import { callerFromVerified, DatabaseId, } from "../internal/authorization/index.js";
import { authenticateRequest } from "./admit.js";
import { JwtVerifier, fromEnv } from "./jwt.js";
import { dbPrefix, prefixedBucket } from "../internal/storage/index.js";
import { armCheckpoint, checkpointStatus, enableTestHooks, isCheckpointReleaseDelay, MAX_CHECKPOINT_RELEASE_DELAY_MS, releaseCheckpoint, } from "../internal/test-hooks.js";
import { internalHeaders } from "../internal/transactor/internal.js";
import { TEST_SESSION_TOKEN_HEADER } from "./session.js";
import { BadRequest, Internal, NotFound, UpstreamError } from "./errors.js";
import { basisHeaders, coloHeader, fetchBasisWithStats, invalidateBasis, nearestReplica, replicationRevisionStoreId, } from "./peer.js";
import { handleStorageTestAdmin } from "./storage-test-admin.js";
import { clearServerIdentityRootCache, serverIdentityRoot, serverIdentityRootId, } from "./server-identity.js";
const json = (body, status = 200, headers = {}) => new Response(JSON.stringify(body), {
    status,
    headers: { "content-type": "application/json", ...headers },
});
const TEST_PREFIX = "/__test__/db/";
export const parseTestAdminPath = (pathname) => {
    if (!pathname.startsWith(TEST_PREFIX))
        return undefined;
...
const bytesOf = (bodyBase64) => {
    if (bodyBase64 === undefined || bodyBase64.length === 0)
...
const handleR2 = async (request, env, db) => {
    ...
    if (action === "put") {
        if (key.length === 0)
            throw new BadRequest({ message: "r2 put needs key" });
        await bucket.put(key, bytesOf(typeof body.bodyBase64 === "string" ? body.bodyBase64 : undefined));
        return json({ ok: true, key });
    }
    if (action === "get") {
        if (key.length === 0)
            throw new BadRequest({ message: "r2 get needs key" });
        const obj = await bucket.get(key);
        if (obj === null)
            return json({ ok: true, key, found: false });
        return json({ ok: true, key, found: true, bodyBase64: b64Of(await obj.arrayBuffer()) });
    }
    if (action === "head") {
        if (key.length === 0)
            throw new BadRequest({ message: "r2 head needs key" });
        const obj = await bucket.head(key);
        return json({ ok: true, key, found: obj !== null });
    }
    if (action === "delete") {
        if (key.length === 0)
            throw new BadRequest({ message: "r2 delete needs key" });
...
const handleCheckpointLocal = (body) => {
    ...
    if (name.length === 0)
        throw new BadRequest({ message: "checkpoint needs name" });
    if (action === "arm-wait") {
...
const transactorUrl = (db, path) => `https://transactor${path}${path.includes("?") ? "&" : "?"}db=${encodeURIComponent(db)}`;
const forwardTransactorSubscription = (request, env, db, from) => env.TRANSACTOR.get(env.TRANSACTOR.idFromName(db)).fetch(transactorUrl(db, `/subscribe?from=${from}`), {
    headers: {
        Upgrade: "websocket",
        ...coloHeader(request),
        ...internalHeaders(env),
...
const testBasisOptions = (request) => {
...
const forward = async (request, env, db, scope, path, body, opts = {}) => {
    const headers = {
...
const entityIdScopeOfBody = (value) => {
...
const handleServerIdentity = async (request, env, db) => {
    const body = (await request.json());
    ...
    if (body.action !== "probe" &&
        ...
        throw new BadRequest({
            message: "server-identity action must be probe|forget-isolate-cache|seal-entity-id|open-entity-id|invocation-entity-id-scope",
        });
    }
    ...
    if (body.action === "invocation-entity-id-scope") {
        ...
        const verified = await Effect.runPromise(authenticateRequest(new Request(request.url, {
    ...
    if (body.action === "seal-entity-id") {
        ...
        return json({
            ok: true,
            keyId: root.keyId,
            token: await sealEntityId(sealingKeyOf(root), entityIdScopeOfBody(body.scope), body.eid),
        });
    }
    if (body.action === "open-entity-id") {
        ...
        return json({
            ok: true,
            keyId: root.keyId,
            resolution: await openEntityId(sealingKeyOf(root), entityIdScopeOfBody(body.scope), body.token),
        });
    }
    return json({
        ok: true,
...
const handleReplicationRevision = async (request, env, db) => {
    const body = (await request.json());
    ...
    const response = await env.REPLICA.get(replicationRevisionStoreId(env, db, body.binding)).fetch(`https://replica/replication/revision?db=${encodeURIComponent(db)}`, {
        method: "POST",
...
export const handleTestAdmin = async (request, env, url) => {
    enableTestHooks();
    ...
    if (rest === "/watch") {
        if (request.method !== "GET")
            throw new BadRequest({ message: "test watch is GET" });
        ...
        if (health.protocol === "ws:")
            health.protocol = "http:";
        if (health.protocol === "wss:")
    ...
    if (rest === "/session") {
        if (request.method !== "GET")
            throw new BadRequest({ message: "test session is GET" });
        ...
        return nearestReplica(env, db, request).fetch(`https://replica/session?db=${encodeURIComponent(db)}`, { headers });
    }
    if (request.method !== "POST")
        throw new BadRequest({ message: "test admin is POST" });
    if (rest === "/r2")
        return handleR2(request, env, db);
    if (rest === "/server-identity")
        return handleServerIdentity(request, env, db);
    if (rest === "/replication-revision") {
        return handleReplicationRevision(request, env, db);
    }
    if (rest === "/storage")
        return handleStorageTestAdmin(request, env, db);
    if (rest === "/basis") {
        const body = (await request.json());
    ...
    if (rest === "/checkpoint") {
        ...
        const scope = body.scope === "transactor" || body.scope === "replica" || body.scope === "worker"
            ? body.scope
            : "worker";
        if (scope === "worker")
            return handleCheckpointLocal(body);
    ...
    if (rest === "/operation-receipts") {
        return forward(request, env, db, "transactor", "/admin/test/operation-receipts", "{}", { passThrough: true });
    }
    if (rest === "/log") {
        const body = (await request.json());
        ...
        return forward(request, env, db, "transactor", `/log?from=${from}&to=${to}`, "{}", { passThrough: true });
    }
    if (rest === "/transact") {
...
export const asTestAdminError = (err) => {
