import { INVOCATION_RECEIPT_VERSION } from "../internal/authorization/invocation-receipts.js";
export const PUBLIC_OBSERVATION_ALLOWLIST = Object.freeze({
    healthFields: Object.freeze(["ok", "service"]),
...
const RESPONSE_HEADERS = new Set(PUBLIC_OBSERVATION_ALLOWLIST.responseHeaders);
const ERROR_FIELDS = new Set(PUBLIC_OBSERVATION_ALLOWLIST.errorFields);
...
export const publicErrorBody = (body) => {
    const out = {};
    for (const [name, value] of Object.entries(body)) {
        if (!ERROR_FIELDS.has(name) || value === undefined)
            continue;
        if (name === "receipt") {
...
export const publicResponseHeaders = (headers) => {
...
const configuredOrigins = (env) => (env.RAMOSE_ALLOWED_ORIGINS ?? "")
    .split(",")
    .map((origin) => origin.trim())
    .filter((origin) => origin.length > 0);
export const publicCorsHeaders = (request, env) => {
    ...
    return {
        ...
        "access-control-allow-headers": PUBLIC_OBSERVATION_ALLOWLIST.requestHeaders.join(","),
...
//# sourceMappingURL=public-observation.js.map