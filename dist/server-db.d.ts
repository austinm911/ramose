...
type ServerEffectRead<C extends AnySchema> = Omit<EffectReadDb<C>, "live" | "livePull" | "asOf" | "history"> & {
    asOf(t: number): ServerEffectRead<C>;
    readonly history: ServerEffectRead<C>;
};
type ServerEffectDb<C extends AnySchema> = Omit<EffectDb<C>, "live" | "livePull" | "asOf" | "history"> & {
    asOf(t: number): ServerEffectRead<C>;
    readonly history: ServerEffectRead<C>;
};
/** `ReadDb` without streaming methods — the server-side read view. */
export type ServerReadDb<C extends AnySchema = AnySchema> = Omit<ReadDb<C>, "live" | "livePull" | "effect" | "asOf" | "history"> & {
...
export type ServerDb<C extends AnySchema = AnySchema> = Omit<Db<C>, "live" | "livePull" | "effect" | "asOf" | "history"> & {
    asOf(t: number): ServerReadDb<C>;
    readonly history: ServerReadDb<C>;
...
export declare const withoutLive: <C extends AnySchema>(db: Db<C>) => ServerDb<C>;
/** Type-level read-only view of a server handle. Writes still exist at runtime. */
export declare const asRead: <C extends AnySchema>(db: ServerDb<C>) => ServerReadDb<C>;
export {};
