import { ValueTag } from "../core/datom.js";
import { base64ToBytes } from "../core/log.js";
import { DB_DOC, Schema, VALUE_TYPE_IDENTS, VALUE_TYPE_NAMES, attributeDatoms, bootstrapDatoms, } from "../core/schema.js";
export const REPLICA_USER_T = 2;
const withoutDocumentation = (datoms) => datoms.filter((datom) => datom.a !== DB_DOC);
export const replicaBootstrapDatoms = () => withoutDocumentation(bootstrapDatoms());
export const replicaAttributeDatoms = (e, spec, t) => withoutDocumentation(attributeDatoms(e, spec, t));
export const replicaAttributes = (attributes) => {
    const seen = new Set();
    const bootstrap = Schema.bootstrap();
    return Object.freeze([...attributes]
        .sort((left, right) => left.ident < right.ident ? -1 : left.ident > right.ident ? 1 : 0)
        .map((spec) => {
        if (!spec.ident.startsWith(":"))
            throw new Error(`invalid replica attribute ${spec.ident}`);
        if (seen.has(spec.ident))
            throw new Error(`duplicate replica attribute ${spec.ident}`);
        seen.add(spec.ident);
...
const canonicalAttribute = (spec) => JSON.stringify([
    spec.ident,
    spec.valueType,
    spec.cardinality,
    spec.index,
...
export const sameReplicaAttributes = (left, right) => left.length === right.length &&
    left.every((spec, index) => {
        const other = right[index];
        return other !== undefined && canonicalAttribute(spec) === canonicalAttribute(other);
    });
export const replicaValueTag = (value) => {
    switch (value.type) {
...
export const replicaDatomValue = (value, entities) => {
    switch (value.type) {
        case "double":
...
export const replicaFactDatom = (logical, schema, entities) => {
...
export const replicaSchemaDatoms = (attributes, attributeIds) => {
    const bootstrap = Schema.bootstrap();
    const datoms = [];
...
export const replicaSchema = (attributes, attributeIds) => {
