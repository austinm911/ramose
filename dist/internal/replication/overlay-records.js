import { isClientRef, isEntityId, isInvocationId, } from "../../db/refs.js";
import { isAllocationSlotName } from "../../db/allocations.js";
import { resolveProjectionBinding, } from "./projection-binding.js";
import { decideQuarantine, mutationPartitionKey, parseMutationPartitionKey, sameSealingEpoch, sealingEpochOf, } from "./outbox.js";
...
const sealedHandles = (target, refs) => {
    const handles = [];
...
const embeddedEpoch = (target, refs) => {
    let epoch = null;
...
export const suppliedRefs = (record) => {
    const refs = [];
    const seen = new Set();
    const add = (ref) => {
        if (seen.has(ref))
...
export const buildOptimisticLayer = (draft) => {
    const { record } = draft;
    ...
    const built = Object.freeze({
        ...
        input: record.input,
        allocations: Object.freeze(record.allocations.map((allocation) => Object.freeze({ ...allocation }))),
        refs,
...
export const withLayerState = (record, state, activation) => durable(Object.freeze({ ...record, state, activation }));
...
export const decodeOptimisticLayer = (value) => {
    if (!isPlainObject(value))
        return undefined;
    if (typeof value.partition !== "string" ||
        parseMutationPartitionKey(value.partition) === undefined)
        return undefined;
    if (!isSequence(value.sequence) || !isInvocationId(value.invocation))
        return undefined;
    ...
    if (!isPlainObject(value.operation) || typeof value.operation.catalog !== "string" ||
        ...
        return undefined;
    if (typeof value.operationVersion !== "string" ||
    ...
    if (target === undefined)
        return undefined;
    if (!Array.isArray(value.allocations) || !Array.isArray(value.refs))
        return undefined;
    const allocations = [];
    const slots = new Set();
    ...
    for (const ref of value.refs) {
        ...
        if (supplied.has(ref) || claimed.has(ref))
            return undefined;
        supplied.add(ref);
        refs.push(ref);
    }
    if (value.state !== "queued" && value.state !== "committed-unobserved")
        return undefined;
    if (typeof value.activation !== "number" || !Number.isSafeInteger(value.activation) ||
        value.activation < 0)
        return undefined;
    if (value.state === "queued" && value.activation !== 0)
        return undefined;
...
const decodeSealing = (value) => {
    ...
    if (!isPlainObject(value))
        return undefined;
    if (typeof value.codecVersion !== "number" ||
...
const cloneJson = (value, seen) => {
    ...
    switch (typeof value) {
        ...
        case "number":
            if (!Number.isFinite(value))
                throw new TypeError("non-finite input");
            return value;
        case "object":
            break;
        default:
    ...
    if (Array.isArray(object)) {
        ...
        for (let index = 0; index < object.length; index++) {
            if (!(index in object))
                throw new TypeError("sparse input");
        ...
        copy = Object.freeze(Object.fromEntries(Object.entries(object).map(([key, item]) => [key, cloneJson(item, seen)])));
    }
    seen.delete(object);
    return copy;
};
export const restoreOverlayLayers = (rows, replay) => {
    const quarantined = [];
    const layers = [];
    const ordered = [...rows.layers].sort((left, right) => left.sequence - right.sequence);
    if (rows.unreadable > 0) {
        quarantined.push(Object.freeze({ invocation: undefined, reason: "unreadable-row" }));
    }
    ...
    return Object.freeze({ type: "layers", layers: Object.freeze(layers) });
};
export const layerOf = (record, changeset) => Object.freeze({
    invocation: record.invocation,
    sequence: record.sequence,
    state: record.state,
...
export const declaredRefs = (record) => Object.freeze([
    ...record.allocations.map((allocation) => allocation.clientRef),
    ...record.refs,
]);
//# sourceMappingURL=overlay-records.js.map