import { IndexedDbReplicaStorage, } from "./indexeddb.js";
import { ReplicaCorruptError, replicaRefused, restoredReplica, } from "./replica-integrity.js";
import { sameReplicationIdentity } from "./state.js";
import { identityInDatabase, isReplicaFenceError, replicaDatabaseKey, replicaDatabaseScopeOf, ReplicaLease, replicaScopeOf, } from "./replica-lifecycle.js";
import { satisfiesActivationFence } from "./activation-fence.js";
import { openReplicationResponse, readReplicationFrames, replicationActivationAddress, replicationCacheSelector, replicationCredentialFingerprint, ReplicationUnauthorizedError, } from "./transport.js";
import { replicaRoutePathKey, replicaRouteScope, replicaRouteSlotFor, stableReplicaRouteSlot, } from "./route-slot.js";
export const classifyReplicationChange = (prior, frame) => {
...
export const classifyReplicationCandidateFrame = (prior, frame) => {
    switch (frame.type) {
        case "Reset":
            return "reset";
        case "SnapshotStart":
...
export const replicationTerminalSnapshot = (frame, value) => Object.freeze({
    status: "terminal",
    terminalCode: frame.code,
    ...(value === undefined ? {} : { value }),
});
const identityOf = (frame) => "identity" in frame ? frame.identity : undefined;
const valueFrom = (identity, replica, stale) => Object.freeze({
    db: replica.db,
    identity,
...
const sessionRegistration = (storage, identity, close) => {
...
export class ReplicationSession {
    ...
    fenced = false;
    constructor(storage, attributes, readCompatibilityHash, onActivationOutcome, initial, lease, registration, retention, run) {
        this.storage = storage;
        this.attributes = attributes;
        this.readCompatibilityHash = readCompatibilityHash;
        this.onActivationOutcome = onActivationOutcome;
        this.lease = lease;
    ...
    refreshFromDurable() {
        const next = this.refreshing.then(() => this.readDurableHead(), () => this.readDurableHead());
        this.refreshing = next.then(() => undefined, () => undefined);
        return next;
    }
    async readDurableHead() {
        ...
        const restored = restoredReplica(await this.storage.restoreOutcome(identity, this.attributes, this.readCompatibilityHash));
    ...
    async revalidate() {
        ...
        const generation = this.generation;
        if (identity === undefined || !this.current(generation))
            return false;
        try {
            ...
            if (!isReplicaFenceError(error) || !this.current(generation))
                return false;
    ...
    untrack() {
        for (const release of this.tracking)
            release();
        this.tracking = [];
    ...
    static async open(options) {
        ...
        const routeSlot = observedSlot ?? await replicaRouteSlotFor({
        ...
        const candidateKey = selector === undefined
            ? undefined
            : { selector, routeSlot };
        const restored = restoredReplica(await options.storage.restoreBoundOutcome(fingerprint, options.attributes, options.readCompatibilityHash));
        let live;
        let closedBeforeStart = false;
        const registration = restored === undefined ? undefined : sessionRegistration(options.storage, restored.identity, async () => {
        ...
        try {
            candidate = restored === undefined && candidateKey !== undefined
                ? await options.storage.selectCacheCandidate(candidateKey, options.readCompatibilityHash)
                : undefined;
            ...
            for (const release of registration?.releases ?? [])
                release();
            retention?.();
            throw error;
        }
        const session = new ReplicationSession(options.storage, options.attributes, options.readCompatibilityHash, options.onActivationOutcome, restored, lease, registration, retention, async (session, generation) => {
            ...
            try {
                    ...
                    if (frameIdentity !== undefined) {
                        ...
                        if (!bindingConfirmed) {
                            ...
                            const confirmedFingerprint = confirmedSlot === routeSlot
                                ? fingerprint
                                : await replicationCredentialFingerprint(options.credential, activation, confirmedSlot);
                            ...
                            if (session.state.value === undefined) {
                                const terminal = await session.acceptConfirmedInitial(frame, prior, action, generation);
                                if (terminal)
                                    return;
                                continue;
                            }
                ...
                if (session.controller.signal.aborted || !session.current(generation))
                    return;
    ...
    async settled(frame, generation) {
        const hook = this.onActivationOutcome;
        if (hook === undefined || this.fenced || !this.current(generation))
            return;
        if (!satisfiesActivationFence(frame.type))
            return;
        this.fenced = true;
        try {
            await hook();
    ...
    quarantine(generation) {
        if (!this.current(generation) || this.state.value === undefined)
            return;
        this.publish({ status: "connecting" });
    }
    publishReplica(identity, replica, stale, generation) {
        if (!this.current(generation)) {
            replica.release();
            return;
        }
    ...
    publishStale(identity, replica, generation) {
        if (!this.current(generation)) {
            replica.release();
            return;
        }
    ...
    async confirmedCandidate(prior) {
        return this.confirmed(await this.storage.restoreCandidateOutcome(prior, this.attributes, this.readCompatibilityHash));
    }
    confirmed(outcome) {
    ...
    async acceptConfirmedInitial(frame, prior, action, generation) {
        switch (action) {
            case "resume":
            case "duplicate": {
                if (prior === undefined ||
            ...
            case "reset":
            case "snapshot": {
                if ((frame.type !== "Reset" && frame.type !== "SnapshotStart") ||
                    (action === "reset" && frame.type !== "Reset") ||
                    (action === "snapshot" && frame.type !== "SnapshotStart")) {
                    throw new Error("authenticated candidate action disagrees with its frame");
                }
                const restored = restoredReplica(await this.storage.restoreOutcome(frame.identity, this.attributes, this.readCompatibilityHash));
                if (!this.current(generation)) {
                    restored?.release();
                    return false;
                }
    ...
    async accept(frame, generation) {
        ...
        switch (frame.type) {
            ...
            case "SnapshotChunk":
                ...
                return false;
            case "SnapshotCommit": {
                const installed = await this.storage.commitSnapshot(frame, this.attributes, { signal: this.controller.signal, lease: this.lease });
            ...
            case "ResumeReady": {
                const prior = this.state.value;
                if (prior === undefined || prior.revision !== frame.revision ||
                    !sameReplicationIdentity(prior.identity, frame.identity)) {
