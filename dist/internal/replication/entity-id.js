...
const DENIED = Object.freeze({ type: "denied" });
const quarantine = (reason) => Object.freeze({ type: "update-required", reason });
const canonicalScope = (scope) => canonicalizeJson({
...
const deriveKeys = async (material, scopeText) => {
    const root = await crypto.subtle.importKey("raw", utf8.encode(material), "HKDF", false, ["deriveBits"]);
...
const keysFor = (sealing, scopeText) => {
    ...
    const pending = deriveKeys(sealing.material, scopeText).catch((cause) => {
        if (derived.get(cacheKey) === pending)
...
const keyIdBytes = (sealing) => {
    const bytes = decodeBase64Url(sealing.keyId);
    if (bytes === undefined || bytes.length !== KEY_ID_BYTES) {
        throw new TypeError("ramose/replication: the sealing key id is not 16 base64url bytes");
    }
    return bytes;
};
const additionalData = (keyId, scopeText) => utf8.encode(`${AAD_DOMAIN}\u0000${ENTITY_ID_CODEC_VERSION}\u0000${base64Url(keyId)}\u0000${scopeText}`);
const encodeEid = (eid) => {
    if (!Number.isSafeInteger(eid) || eid < 0) {
        throw new TypeError("ramose/replication: an entity id must be a non-negative safe integer");
    }
    const bytes = new Uint8Array(EID_BYTES);
    new DataView(bytes.buffer).setBigUint64(0, BigInt(eid));
    return bytes;
};
const syntheticIv = async (keys, aad, eidBytes) => {
...
const keystream = async (keys, siv, bytes) => new Uint8Array(await crypto.subtle.encrypt({ name: "AES-CTR", counter: siv, length: 64 }, keys.cipher, bytes));
...
const decodeCanonicalBase64Url = (text) => {
    const bytes = decodeBase64Url(text);
    if (bytes === undefined || base64Url(bytes) !== text)
        return undefined;
    return bytes;
};
const sealWith = async (sealing, scopeText, keys, eidBytes) => {
    const keyId = keyIdBytes(sealing);
    const siv = await syntheticIv(keys, additionalData(keyId, scopeText), eidBytes);
...
export const sealEntityId = async (sealing, scope, eid) => {
    const eidBytes = encodeEid(eid);
    const scopeText = canonicalScope(scope);
    return sealWith(sealing, scopeText, await keysFor(sealing, scopeText), eidBytes);
};
export const openEntityId = async (sealing, scope, token) => {
    const envelope = decodeCanonicalBase64Url(token);
    if (envelope === undefined)
        return DENIED;
    if (envelope.length < PREAMBLE_BYTES)
        return DENIED;
    if (envelope[0] !== ENTITY_ID_CODEC_VERSION)
        return quarantine("codec-version");
    ...
    if (!equalBytes(await syntheticIv(keys, additionalData(keyId, scopeText), eidBytes), siv)) {
        return DENIED;
    }
    const eid = new DataView(eidBytes.buffer, eidBytes.byteOffset, EID_BYTES).getBigUint64(0);
    if (eid > BigInt(Number.MAX_SAFE_INTEGER))
        return DENIED;
