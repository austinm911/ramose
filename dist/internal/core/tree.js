 ...
 */
import { ByteReader, ByteWriter, fromHex, toHex } from "./bytes.js";
import { COMPARATORS, Index, ValueTag, comparePrefix, } from "./datom.js";
import { decodeSegment, encodeSegment, readDatom, writeDatom } from "./segment.js";
// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------
export const NodeKind = { Leaf: 0, Dir: 1 };
...
export function encodeNode(index, node) {
    if (node.kind === NodeKind.Leaf)
        return encodeSegment(index, node.datoms);
...
export function decodeNode(buf) {
...
export function lowerBound(index, datoms, p) {
    let lo = 0, hi = datoms.length;
    while (lo < hi) {
        const mid = (lo + hi) >>> 1;
        if (comparePrefix(index, datoms[mid], p) < 0)
            lo = mid + 1;
        else
            hi = mid;
    }
...
/** First index i in [0,n) with comparePrefix(datoms[i], p) > 0, else n. */
export function upperBound(index, datoms, p) {
    let lo = 0, hi = datoms.length;
    while (lo < hi) {
        const mid = (lo + hi) >>> 1;
        if (comparePrefix(index, datoms[mid], p) <= 0)
 ...
 */
export function prefixComparator(index, p) {
    const hasE = p.e !== undefined, hasA = p.a !== undefined, hasV = p.vt !== undefined, hasT = p.t !== undefined;
    if (!hasV && !hasT) {
        switch (index) {
            case Index.EAVT:
                if (hasE && !hasA)
                    return (d, q) => (d.e < q.e ? -1 : d.e > q.e ? 1 : 0);
                if (hasE && hasA)
                    return (d, q) => (d.e < q.e ? -1 : d.e > q.e ? 1 : d.a < q.a ? -1 : d.a > q.a ? 1 : 0);
                break;
            case Index.AEVT:
                if (hasA && !hasE)
                    return (d, q) => (d.a < q.a ? -1 : d.a > q.a ? 1 : 0);
                if (hasA && hasE)
                    return (d, q) => (d.a < q.a ? -1 : d.a > q.a ? 1 : d.e < q.e ? -1 : d.e > q.e ? 1 : 0);
                break;
        }
    }
...
export function lowerBoundFrom(cmp, datoms, p, from) {
...
export function upperBoundFrom(cmp, datoms, p, from) {
    const n = datoms.length;
    let lo = from, step = 1, hi = from;
    while (hi < n && cmp(datoms[hi], p) <= 0) {
        lo = hi + 1;
...
export function lowerBoundDatom(cmp, datoms, d) {
    let lo = 0, hi = datoms.length;
    while (lo < hi) {
        const mid = (lo + hi) >>> 1;
        if (cmp(datoms[mid], d) < 0)
            lo = mid + 1;
        else
            hi = mid;
    }
...
function childForPrefix(index, dir, p) {
    // largest i with keys[i] < p (cmp<0); else 0
    const keys = dir.keys;
    let lo = 0, hi = keys.length;
    while (lo < hi) {
        const mid = (lo + hi) >>> 1;
        if (comparePrefix(index, keys[mid], p) < 0)
            lo = mid + 1;
        else
            hi = mid;
    }
...
async function getNode(src, ref) {
 ...
 */
export async function* scan(src, index, root, prefix) {
    ...
    for (;;) {
        ...
        if (node.kind === NodeKind.Leaf) {
            ...
            if (s < ds.length) {
                ...
                if (s < e)
                    yield { datoms: ds, start: s, end: e };
                if (e < ds.length)
                    return; // hit a datom past the prefix
            }
            break;
        }
    ...
    for (;;) {
        ...
        if (comparePrefix(index, d.keys[i], prefix) > 0)
            return; // subtree entirely past prefix
        let child = src.peek(d.refs[i].hash);
        if (child === undefined)
            child = await src.load(d.refs[i]);
        // descend leftmost through dirs
        while (child.kind === NodeKind.Dir) {
            dirs.push(child);
            idxs.push(1);
            const cref = child.refs[0];
 ...
 */
export async function scanMany(src, index, root, prefixes) {
    const results = new Array(prefixes.length);
    ...
    for (const i of order) {
        ...
        for (;;) {
            ...
            lastEnd = e;
            if (e < ds.length)
                break; // hit a datom past p (or found nothing in a leaf that spans p)
 ...
 */
function sortedPrefixOrder(index, prefixes, asDatom) {
    const n = prefixes.length;
    const cmp = (x, y) => comparePrefix(index, asDatom[x], prefixes[y]);
        ...
        for (let i = 0; i < n; i++) {
            ...
            switch (index) {
                ...
                case Index.AVET:
                    ...
                    break;
                default:
                    k = p.vt === ValueTag.Ref ? p.v : undefined;
                    break;
            }
            if (typeof k !== "number" || !(k >= 0 && k < LIMIT) || !Number.isInteger(k)) {
                ok = false;
                break;
            }
            packed[i] = k * 2 ** 20 + i;
...
function prefixAsDatom(p) {
    return { e: p.e ?? 0, a: p.a ?? 0, vt: (p.vt ?? 0), v: (p.v ?? 0), t: p.t ?? 0, op: true };
}
/** First datom matching the prefix, or undefined. */
export async function seekOne(src, index, root, prefix) {
...
export async function collect(src, index, root, prefix) {
    const out = [];
    for await (const c of scan(src, index, root, prefix)) {
 ...
 */
export async function estimateCount(src, index, root, prefix) {
    ...
    const locate = (d) => {
        ...
        while (lo < hi) {
            const mid = (lo + hi) >>> 1;
            if (comparePrefix(index, d.keys[mid], prefix) <= 0)
                lo = mid + 1;
            else
                hi = mid;
        }
    ...
    for (;;) {
        ...
        for (let i = c + 1; i < n.refs.length; i++)
            total += n.refs[i].count;
        lref = n.refs[c];
    }
    ...
    for (;;) {
        ...
        while (lo < hi) {
            const mid = (lo + hi) >>> 1;
            if (comparePrefix(index, n.keys[mid], prefix) <= 0)
                lo = mid + 1;
            else
                hi = mid;
        }
...
async function buildLevels(store, index, entries, fanout) {
    let level = entries;
 ...
 */
export async function buildTree(store, index, sorted, opts = {}) {
...
export function sortedUnion(cmp, a, b) {
    const out = [];
    let i = 0, j = 0;
...
async function mergeNode(store, index, cmp, ref, key, novelty, lo, hi, leafSize, fanout) {
    if (lo >= hi)
        return [{ key: key, ref }];
    const node = await getNode(store, ref);
    if (node.kind === NodeKind.Leaf) {
        const merged = sortedUnion(cmp, node.datoms, novelty.slice(lo, hi));
        if (merged.length === node.datoms.length)
            return [{ key: key ?? node.datoms[0], ref }]; // all dups
    ...
    for (let i = 0; i < n; i++) {
        ...
        if (start < end) {
            ...
            if (sub.length !== 1 || sub[0].ref.hash !== node.refs[i].hash)
                changed = true;
            for (const e of sub)
                children.push(e);
        }
 ...
 */
export async function mergeTree(store, index, root, novelty, opts = {}) {
    if (novelty.length === 0)
        return root;
    const leafSize = opts.leafSize ?? DEFAULT_LEAF;
    const fanout = opts.fanout ?? DEFAULT_FANOUT;
...
export async function treeDepth(src, root) {
    let d = 1;
    let ref = root;
    for (;;) {
        const n = await getNode(src, ref);
...
export async function reachable(src, root, into = new Set()) {
    const stack = [root];
    while (stack.length) {
        const r = stack.pop();
        if (into.has(r.hash))
