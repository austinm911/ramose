...
export function roleIdentOf(principalAttr) {
    return siblingIdentOf(principalAttr, ROLE_NAME);
}
const isStringAttr = (attr) => attr.valueType === ValueTag.Str;
/** Names the peer already owns on this row — attrs must not overwrite them. */
const reservedNames = (principalAttr) => {
    const slash = principalAttr.lastIndexOf("/");
...
const valueMatchesAttr = (attr, value) => {
 ...
 */
export function wantedFacts(policy, principal, db) {
    if (!shouldProvision(principal))
        return undefined;
    const ident = policy.principal;
    const attr = db.attr(ident);
    if (attr === undefined || attr.unique !== "identity")
        return undefined;
    const map = { [ident]: principal.sub };
    const roleIdent = roleIdentOf(ident);
    const roleAttr = db.attr(roleIdent);
    if (roleAttr !== undefined && isStringAttr(roleAttr))
        map[roleIdent] = principal.class;
    ...
    for (const [key, value] of Object.entries(principal.claims.attrs ?? {})) {
        ...
        if (siblingAttr === undefined || !valueMatchesAttr(siblingAttr, value))
            continue;
        map[sibling] = value;
    }
    return map;
 ...
 */
export async function provisionTx(policy, principal, db) {
    const map = wantedFacts(policy, principal, db);
    if (map === undefined)
        return undefined;
    const eid = await db.entid([policy.principal, principal.sub]);
    if (eid !== undefined) {
        const ent = await db.entity(eid);
        if (ent !== undefined && factsMatch(ent, map))
            return undefined;
    }
    return [map];
}
/** Resolve `sub` through the policy's principal attr. */
export async function resolveProvisionedEid(policy, principal, db) {
    if (!shouldProvision(principal))
        return principal.eid;
    if (principal.eid !== undefined)
        return principal.eid;
    return db.entid([policy.principal, principal.sub]);
}
