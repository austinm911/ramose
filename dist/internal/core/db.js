import { Index, ValueTag, inferTag, normalizeValue, } from "./datom.js";
import { Novelty, collapseCurrent, currentView, filterAsOf, mergeChunks, rawView } from "./novelty.js";
import { DB_IDENT, Schema, isTxEid } from "./schema.js";
import { estimateCount, scan, scanMany, sortedUnion } from "./tree.js";
import { COMPARATORS } from "./datom.js";
export function rootFor(roots, index) {
    switch (index) {
        case Index.EAVT:
            return roots.eavt;
        case Index.AEVT:
...
export class Db {
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            if (hasNovelty) {
                ...
                if (n !== undefined) {
                    ...
                    ds = ds.length === 0 ? nds : sortedUnion(cmp, ds, nds);
                }
            }
            if (this.isHistory) {
                if (!rawHistory)
                    ds = ds.filter((d) => d.t <= asOf);
            }
            else if (ds.length > 0) {
    ...
    async datomsArray(index, prefix) {
        const out = [];
        for await (const arr of this.datoms(index, prefix))
            for (const d of arr)
                out.push(d);
        return out;
    }
    async first(index, prefix) {
        for await (const arr of this.datoms(index, prefix))
            return arr[0];
        return undefined;
    }
    async estimate(index, prefix) {
        if (this.filters.length > 0)
            return (await this.datomsArray(index, prefix)).length;
    ...
    async entid(ref) {
        ...
        if (Array.isArray(ref) && ref.length === 2) {
            ...
            if (!attr)
                throw new Error(`lookup ref: unknown attribute ${ref[0]}`);
            if (!attr.unique)
                throw new Error(`lookup ref: attribute ${ref[0]} is not unique`);
    ...
    async entity(e) {
        ...
        for (const d of ds) {
            ...
            const val = d.vt === ValueTag.Inst ? new Date(d.v) : d.v;
    ...
    async identOf(e) {
        ...
        return d === undefined ? undefined : d.v;
    }
    isTx(e) {
        return isTxEid(e);
    }
}
export function coerceValue(vt, value) {
    if (value !== null && typeof value === "object" && "vt" in value && "v" in value) {
        const tv = value;
        if (tv.vt !== vt) {
    ...
    switch (vt) {
        case ValueTag.Inst:
            if (value instanceof Date)
                return { vt, v: value.getTime() };
            if (typeof value === "string") {
                const ms = Date.parse(value);
                if (Number.isNaN(ms))
                    throw new TypeError(`bad instant ${value}`);
                return { vt, v: ms };
            }
            return { vt, v: normalizeValue(vt, value) };
        case ValueTag.Long:
        case ValueTag.Ref:
            if (typeof value === "bigint")
                return { vt, v: normalizeValue(vt, Number(value)) };
            return { vt, v: normalizeValue(vt, value) };
...
export function datomJsValue(d) {
    return d.vt === ValueTag.Inst ? new Date(d.v) : d.v;
}
