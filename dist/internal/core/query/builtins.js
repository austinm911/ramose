 ...
 */
import { compareStrings } from "../datom.js";
...
export function vkey(v) {
    switch (typeof v) {
        ...
        case "object": {
            ...
            const o = v;
            if ("vt" in o && "v" in o)
                return vkey(o.v); // TaggedValue → payload key
            if (Array.isArray(v))
                return "[" + v.map(vkey).join(",") + "]";
...
export function sortKeys(order, col) {
 ...
 */
export function compareCells(x, y, k) {
...
export const FUNCTIONS = {
    "+": (...xs) => xs.reduce((a, b) => a + num(b), 0),
    "-": (a, ...xs) => (xs.length === 0 ? -num(a) : xs.reduce((acc, b) => acc - num(b), num(a))),
    "*": (...xs) => xs.reduce((a, b) => a * num(b), 1),
    "/": (a, ...xs) => xs.reduce((acc, b) => acc / num(b), num(a)),
    ...
    "clojure.string/join": (sep, xs) => (Array.isArray(sep) ? sep.join("") : xs.join(String(sep))),
    rand: (n) => (n === undefined ? Math.random() : Math.random() * num(n)),
...
export const AGGREGATES = {
    ...
    max: (vs, n) => n === undefined ? (vs.length ? sorted(vs)[vs.length - 1] : null) : distinctSorted(vs).slice(-num(n)).reverse(),
    median: (vs) => {
        if (!vs.length)
            return null;
