 ...
 */
import { Connection, bootstrapDatoms, decodeLogChunk, emptyRoots, encodeLogChunk, fromJson, gzipCodec, toJson, txFrame, FIRST_USER_EID, Histogram, RateMeter, TxError, checkTx, publicPolicyOp, componentLogger, filterDb, canChangeSchema, isSchemaTx, isSuperuser, provisionTx, resolveProvisionedEid, shouldProvision, toWireDatom, } from "../core/index.js";
import { FilteredDb } from "../core/policy/filter.js";
import { R2NodeStore, readCurrentRoot, recordToRoots, rootsToRecord } from "../storage/index.js";
import * as Effect from "effect/Effect";
import { BadRequest, NotFound, TransactorDeadError, TxRejected, errorResponse, toHttpError } from "./errors.js";
import {} from "./host.js";
import { asPrincipal } from "./policy.js";
...
export class Transactor {
    ...
    async boot() {
        ...
        for (const row of this.getMeta("op_acks") ?? []) {
    ...
    lookupOpAck(principal, clientOpId) {
            ...
            while (this.queue.length > 0 && this.dead === undefined) {
                ...
                for (const p of batch) {
                    ...
                    try {
                        ...
                        const txInstant = rep.txData[0]?.v; // :db/txInstant is first
                ...
                try {
                    ...
                    this.host.transactionSync(() => {
                        ...
                        if (persist.length > 0) {
                            for (const [id, ack] of persist)
                                this.recentOpAcks.set(id, ack);
                            this.setMeta("op_acks", [...this.recentOpAcks.entries()]
                                .map(([k, a]) => ({ k, ack: a }))
                ...
                for (const [id, ack] of batchAcks)
                    this.rememberAck(id, ack, ack.output !== undefined);
                for (const a of acks)
                    a.p.resolve(a.ack);
     ...
     */
    async applyProvision(p, entries) {
        const policy = this.host.policy;
        const who = p.principal;
        if (!policy || !who || !shouldProvision(who))
            return;
        const ops = await provisionTx(policy, who, this.conn.db());
        if (ops !== undefined) {
            const rep = await this.conn.transact(ops);
            entries.push({ t: rep.t, txInstant: rep.txData[0]?.v, datoms: rep.txData });
        }
    ...
    async authorize(p) {
        ...
        if (p.system)
            return p.tx;
        if (!p.principal)
            throw new TxRejected({ message: "no principal", code: "policy" });
        if (isSuperuser(p.principal, policy))
            return p.tx;
        if (p.fromOperation)
            return p.tx;
        if (isSchemaTx(p.tx) && canChangeSchema(p.principal, policy))
            return p.tx;
        ...
        if (who.eid === undefined && who.sub !== undefined) {
            const eid = await db.entid([policy.principal, who.sub]);
            if (eid !== undefined)
                who = { ...who, eid };
        }
        const res = await checkTx(p.tx, db, policy, who);
        if (!res.ok)
            throw new TxRejected({ message: `${publicPolicyOp(res.op)} denied on ${res.attr}`, code: res.code, attr: res.attr });
        return res.ops;
    }
    /** Ack facts the writer may read, judged against the post-commit unfiltered db. */
    async ackDatoms(datoms, principal) {
        const policy = this.host.policy;
        ...
        for (const d of datoms)
            if (await view.visible(d))
                kept.push(toWireDatom(d));
        return kept;
    }
    /** A unique conflict names the entity and value it collided with — a read leak under a policy. */
    scrub(err, p) {
        if (!this.host.policy || !p.principal || isSuperuser(p.principal, this.host.policy))
            return err;
        if (err instanceof TxError && err.code === "tx/unique-conflict")
            return new TxRejected({ message: "unique conflict", code: err.code });
        return err;
    }
