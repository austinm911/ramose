...
export const OPERATION_VERSION_DESCRIPTOR_VERSION = 2;
export const DEFAULT_OPERATION_REVISION = 1;
export const requireOperationRevision = (value, label) => {
    if (value === undefined)
        return DEFAULT_OPERATION_REVISION;
    if (typeof value !== "number" || !Number.isSafeInteger(value) || value < 1) {
        throw new Error(`ramose: ${label} revision must be a positive integer, not ${JSON.stringify(value)}`);
    }
    return value;
};
...
const stripSchemaDocumentation = (node) => {
    if (Array.isArray(node))
        return node.map(stripSchemaDocumentation);
...
const definitionRefName = (value, names) => {
...
const orderDefinitionNames = (node, definitions, names, order) => {
    if (Array.isArray(node)) {
        for (const item of node)
            orderDefinitionNames(item, definitions, names, order);
        return;
    }
    if (!isJsonObject(node))
        return;
...
const rewriteDefinitionRefs = (node, names, renamed) => {
    if (Array.isArray(node)) {
...
const canonicalizeDefinitionNames = (document, mapKey) => {
    const definitions = document[mapKey];
...
export const normalizeContractRepresentation = (representation) => {
    ...
    for (const [key, value] of Object.entries(representation)) {
        ...
        else if ((key === "definitions" || key === "$defs") && isJsonObject(value)) {
            if (mapKey === undefined)
                mapKey = key;
            out[key] = Object.fromEntries(Object.entries(value).map(([name, child]) => [
                name,
...
export const operationVersionMaterial = (descriptor) => ({
    ...
    contract: {
        input: {
            representation: normalizeContractRepresentation(descriptor.input.representation),
            shape: descriptor.input.shape,
        },
        output: {
            representation: normalizeContractRepresentation(descriptor.output.representation),
            shape: descriptor.output.shape,
        },
    },
    behavior: {
...
export const hashOperationVersion = Effect.fn("Authorization.hashOperationVersion")(function* (descriptor) {
    const digest = yield* hashDomainSeparatedCanonicalJson(OPERATION_VERSION_DOMAIN_V1, operationVersionMaterial(descriptor));
    return OperationVersion.make(digest);
});
//# sourceMappingURL=operation-version.js.map