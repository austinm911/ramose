...
import { fieldKey, invalid, isBlank, mismatch, operationKey, requireNonBlank, SEPARATOR, } from "./common.js";
import { validateInputShapeKeys } from "./descriptors.js";
const isBlankDoc = (doc) => doc.trim().length === 0;
...
const closeTraits = (edges) => Result.gen(function* () {
    const closed = new Map();
    const visit = (name, active, done) => Result.gen(function* () {
...
const validateFieldRefTarget = (refTarget, target, entities, traits) => Result.gen(function* () {
...
const validateOperationShape = (shape, target, entities, traits, allowSelf, operationName) => Result.gen(function* () {
    switch (shape._tag) {
        case "scalar":
        case "opaque":
            return;
        case "ref":
            if (shape.refTarget._tag === "self" && !allowSelf) {
                return yield* invalid(`targetless operation '${operationName}' cannot reference self`);
            }
            return yield* validateFieldRefTarget(shape.refTarget, target, entities, traits);
        case "array":
            return yield* validateOperationShape(shape.items, target, entities, traits, allowSelf, operationName);
        case "struct":
            yield* Result.all(shape.fields.map((field) => validateOperationShape(field.shape, target, entities, traits, allowSelf, operationName)));
    }
});
export const prepareAuthorizationCatalog = (target, descriptor) => Result.gen(function* () {
    yield* validateTarget(target, descriptor);
    const entities = new Map();
    const traits = new Map();
    const fields = new Map();
    const operations = new Map();
    const entityTraitEdges = new Map();
    const traitTraitEdges = new Map();
    ...
    for (const field of descriptor.fields) {
        ...
        if (field.doc !== undefined && isBlankDoc(field.doc)) {
            return yield* invalid(`blank field doc for '${field.id.owner.name}.${field.id.localName}'`);
        }
        const key = fieldKey(field.id);
        if (fields.has(key))
            return yield* invalid(`ambiguous field '${key}'`);
        const owner = field.id.owner;
        if (owner.kind === "entity") {
            if (!entities.has(owner.name)) {
                return yield* invalid(`missing owner entity '${owner.name}' for field '${field.id.localName}'`);
            }
        }
        else if (!traits.has(owner.name)) {
            return yield* invalid(`missing owner trait '${owner.name}' for field '${field.id.localName}'`);
        }
    ...
    for (const operation of descriptor.operations) {
        ...
        if (operations.has(key))
            return yield* invalid(`ambiguous operation '${key}'`);
        ...
        yield* Result.all([
            validateOperationShape(operation.input, target, entities, traits, operation.id.target === "required", `${owner.name}.${operation.id.localName}`),
            validateOperationShape(operation.output, target, entities, traits, operation.id.target === "required", `${owner.name}.${operation.id.localName}`),
        ]);
        const declaredComposers = new Set();
        ...
        for (const entity of operation.writes) {
            ...
            if (!entities.has(entity.name)) {
                return yield* invalid(`missing operation write entity '${entity.name}'`);
            }
            if (declaredWrites.has(entity.name)) {
                return yield* invalid(`duplicate operation write entity '${entity.name}'`);
            }
            declaredWrites.add(entity.name);
        }
        if (operation.id.target === "none" ||
            operation.id.owner.kind === "entity") {
            if (operation.composers.length !== 0) {
                return yield* invalid(`operation '${owner.name}.${operation.id.localName}' cannot declare composers`);
            }
        }
    ...
    for (const operation of operations.values()) {
        if (operation.id.owner.kind !== "trait")
            continue;
        ...
        if (expected.length === 0) {
            return yield* invalid(`unreachable trait operation owner '${ownerName}' for '${operation.id.localName}'`);
        }
        if (operation.id.target === "none")
            continue;
        ...
        if (declared.length !== expected.length ||
            ...
            return yield* invalid(`contradictory operation composers for '${ownerName}.${operation.id.localName}'`);
        }
    }
    ...
    for (const row of descriptor.traitComposition) {
        ...
        if (seenComposition.has(compositionKey)) {
            return yield* invalid(`duplicate trait composition '${row.composer.name}'/'${row.trait.name}'`);
        }
        ...
        if (computed === undefined || !computed.has(row.trait.name)) {
            return yield* invalid(`entity '${row.composer.name}' does not compose trait '${row.trait.name}'`);
        }
        const expected = new Set([row.trait.name]);
        ...
        if (expected.size !== declared.size) {
            return yield* invalid(`contradictory trait composition for '${row.composer.name}'/'${row.trait.name}'`);
        }
...
export const requireEntity = (index, id, label) => Result.gen(function* () {
    yield* catalogOfIdentity(id, index.target, label);
    const found = index.entities.get(id.name);
    if (found === undefined)
        return yield* invalid(`stale identity: missing ${label} '${id.name}'`);
    return found;
});
export const requireTrait = (index, id, label) => Result.gen(function* () {
    yield* catalogOfIdentity(id, index.target, label);
...
export const requireField = (index, id, label) => Result.gen(function* () {
    yield* catalogOfIdentity(id, index.target, label);
    const found = index.fields.get(fieldKey(id));
    if (found === undefined) {
        return yield* invalid(`stale identity: missing ${label} '${id.owner.kind}:${id.owner.name}.${id.localName}'`);
    }
    return found;
});
export const requireOperation = (index, id, label) => Result.gen(function* () {
    yield* catalogOfIdentity(id, index.target, label);
...
export const entityComposes = (index, entity, traitName) => index.entityTraits.get(entity.name)?.has(traitName) === true;
export const traitComposes = (index, trait, otherName) => trait.name === otherName || index.traitTraits.get(trait.name)?.has(otherName) === true;
export const fieldAccessibleFrom = (index, focus, field) => {
    const owner = field.id.owner;
    if (focus._tag === "entity") {
        if (owner.kind === "entity")
            return owner.name === focus.entity.name;
...
export const ownerFocus = (index, owner) => {
    if (owner.kind === "entity") {
        const entity = index.entities.get(owner.name);
        if (entity === undefined)
            return invalid(`missing owner entity '${owner.name}'`);
        return Result.succeed({ _tag: "entity", entity });
    }
    const trait = index.traits.get(owner.name);
    if (trait === undefined)
        return invalid(`missing owner trait '${owner.name}'`);
    return Result.succeed({ _tag: "trait", trait });
};
export const sameRow = (index, left, right) => {
    ...
    if (left._tag === "trait" && right._tag === "trait") {
        if (left.trait.catalog !== right.trait.catalog)
            return false;
        return (traitComposes(index, left.trait, right.trait.name) ||
