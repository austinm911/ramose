...
import { requireEntity, requireField, requireOperation, requireTrait, } from "./catalog.js";
import { invalid } from "./common.js";
import { meEntity, resourceFocus, walkValue } from "./traversal.js";
import { charge, collectionElement, emptyDerived, eqCompatible, mergeDerived, } from "./types.js";
export const walkExpr = (index, expr, resource, me, classes, claims, limits, spent) => Result.gen(function* () {
    ...
    switch (expr._tag) {
        case "const":
            return derived;
        case "hasClass":
            if (!classes.has(expr.class))
                return yield* invalid(`undeclared class '${expr.class}'`);
            return derived;
        case "and":
        case "or": {
        ...
        case "not": {
            const child = yield* walkExpr(index, expr.expr, resource, me, classes, claims, limits, spent);
        ...
        case "in": {
            ...
            const collection = yield* walkValue(index, expr.collection, resource, me, claims, limits, spent);
...
const compareDerived = (rule, derived, limits) => {
    if (rule.usesResource !== derived.usesResource)
        return invalid("tampered usesResource");
    if (rule.usesMe !== derived.usesMe)
        return invalid("tampered usesMe");
    if (rule.usesSubject !== derived.usesSubject)
        return invalid("tampered usesSubject");
    if (rule.traversalDepth !== derived.traversalDepth)
        return invalid("tampered traversalDepth");
...
const validateFocus = (index, focus) => Result.gen(function* () {
    switch (focus._tag) {
        case "entity":
...
export const validateRule = (index, rule, principal, classes, claims, limits) => Result.gen(function* () {
