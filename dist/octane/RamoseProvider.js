...
import { subSlot } from "./internal.js";
...
export const RamoseProvider = (props) => {
    ...
    const closed = useRef(null, CLOSED);
    // a client a double render discards is harmless: `connect` opens no sockets
    // until a first read, so there is nothing to close
    const client = useMemo(() => connect({ url, token, fetch, webSocket }), [url, token, fetch, webSocket, generation], CLIENT);
    useEffect(() => {
