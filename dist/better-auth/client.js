 ...
 */
import { Unauthorized } from "../db/index.js";
/** The client plugin pairing with `ramoseToken` on the server. */
export const ramoseTokenClient = (options) => {
    const path = options?.path ?? "/ramose/token";
    return {
        id: "ramose-token",
        getActions: ($fetch) => ({
            ramose: {
                token: async (input, fetchOptions) => {
                    ...
                    if (error) {
                        const message = error.message ?? `ramose: token mint failed (${error.status})`;
