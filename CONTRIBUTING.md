...
`RAMOSE_DOCS_DOMAIN` (variable) attaches a custom hostname to the production
docs Worker (the zone must already exist in the account). The existing GitHub
...
(physical Worker name pinned so redeploys adopt the same script — the product
is Ramose and the public site is [ramose.ai](https://ramose.ai)).
Optional: `RAMOSE_DOCS_DOMAIN` (e.g. `ramose.ai`) attaches a custom hostname
once the zone exists in the account. Re-runs update the same Worker
