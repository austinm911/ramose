...
```tsx title="examples/reef/src/app/screens/BoardScreen.tsx:230"
...
`useLive(db, query, params?): { rows, error, ticks }` — a **live query** as React state: it re-runs whenever the database changes ([glossary](/concepts/glossary/#live-query)). Needs no provider. Bind `Ramose.params` as the third argument; a params-only change keeps the last `rows`.
...
```tsx title="examples/reef/src/app/screens/BoardScreen.tsx:453"
...
```tsx title="examples/reef/src/app/screens/BoardScreen.tsx:448"
...
```tsx title="examples/reef/src/app/screens/BoardScreen.tsx:243-245"
