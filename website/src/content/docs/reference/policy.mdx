...
- **Reads are filtered, never rejected.** The server hides the facts you may not read; a list is shorter, a hidden required field makes `pull` return `null` — the same as a missing record. Rules evaluate against the unfiltered data, always at the *current* version, so reading the past cannot revive a revoked grant. The live connection follows the same rule: a write you cannot read never appears on it, and a grant or revoke that changes what you can see asks the page to reload its view.
