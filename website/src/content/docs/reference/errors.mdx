---
title: Errors
description: The eight request errors, plus NotOne — what each means, what to show the user, which are retried for you, and how they map to HTTP status codes.
---
...
Every error is a tagged value and `DbError` is the union of the eight request errors. Most come from the Ramose server — the one Cloudflare Worker that serves all your databases; Ramose's code calls it the *peer*. `.oneOrFail()` can also fail with `NotOne`, which is not a `DbError`. Match by name with `Effect.catchTags`.
