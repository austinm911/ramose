...
The wire, the knobs and the operations for the Ramose server — the one Cloudflare Worker that serves all your databases; Ramose's code calls it the *peer* ([glossary](/concepts/glossary/#server)). This page is for people deploying or operating one; you declare this Worker, you never write it.
...
| route | notes |
| --- | --- |
| `/health` | liveness; needs no token even under a policy |
| `/transact` | the only write; every key in the body is a full field name (`:todo/title`). Optional `clientTxId` is a client-chosen id: sending the same id again returns the original reply and does not write a second time. The response `datoms` is the list of facts that landed (not a count) — **breaking** vs an earlier count. The TypeScript client still exposes `datomCount` as that list's length |
| `/query`, `/pull` | reads at the current version, or at `asOf` / with `history`. `t` is the version read at; `/query` also returns `root`, the newest snapshot behind it, and `explain: true` adds the plan (admin only) |
| `/entity` | every field of one record, at the current version or at `?asOf=`. There is no `history` on this route |
| `/info` | `t` and `principal` for everyone; admins also get writer, read-copy and Worker internals |
| `/session` | the WebSocket connection ([glossary](/concepts/glossary/#connection)) behind `Ramose.connect`: reads travel on it. The server still pushes `{ op: "t", t }` when a write this caller can read lands, so existing clients keep working. It also pushes `{ op: "tx", t, datoms }` with the facts they may read, or `{ op: "resync", t }` when their access changes. A write they cannot read does not appear on the socket at all. A client may send `{ op: "sync", from }` to catch up. Writes are always HTTPS |
...
```ts title="examples/reef/src/infra/resources.ts:41-53"
...
**Bindings** (fixed names): `STORE` — the R2 bucket, **object storage** where every version of every database is kept ([glossary](/concepts/glossary/#object-storage)); `TRANSACTOR` — the writer's Durable Object class `TransactorDO`; `REPLICA` — the read copy's class `QueryReplicaDO`; optional `ANALYTICS` — an Analytics Engine dataset. Compatibility flag `nodejs_compat` is required.
...
| `RAMOSE_INTERNAL_SECRET` | unset | Worker-to-writer gate on every internal call; `authEnv` mints one when a policy is set. Pin it once you split the Worker out |

`Ramose.authEnv({ … })` produces these keys — see [Policy → Server env keys](/reference/policy/).
