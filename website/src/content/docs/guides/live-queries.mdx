...
- **After anyone else's write.** Other windows hear about the new version over their **WebSocket connection** ([glossary](/concepts/glossary/#connection)) and re-run within about a second, locally and deployed. Writes always go over HTTPS; reads and updates share that connection. A write this caller cannot read does not wake the connection. If their access itself changes (they gain or lose a record), the connection says so and the next run sees the new view.
...
```ts title="examples/reef/src/app/components/IssueDetail.tsx:266"
