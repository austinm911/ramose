...
A freshly deployed Ramose peer accepts every request, from anyone, with full
...
- [ ] **Set a policy, or a shared token.** With neither `RAMOSE_POLICY` nor
      `RAMOSE_TOKEN` set, every caller is an administrator on every database.
      Use a [policy](/guides/permissions/) for anything a browser talks to; a
      shared `RAMOSE_TOKEN` is only appropriate when the sole caller is your own
      backend.
- [ ] **Complete the verifier.** A policy needs `RAMOSE_JWKS_URL`,
      `RAMOSE_JWT_ISS`, and `RAMOSE_JWT_AUD`. Pass the same `auth` object to
      `Ramose.Server` and the deploy fails loudly when one is missing — better
      than a peer that denies every request at runtime.
- [ ] **Narrow the origins.** Without a policy, the peer answers CORS for `*`.
      Set `RAMOSE_ALLOWED_ORIGINS` to your own origins. An empty list sends no
...
- [ ] **Check the demo console is gone.** The peer serves a small demo app at
      ...
      you get a 404.
- [ ] **Pin the internal secret.** Set `RAMOSE_INTERNAL_SECRET` so the
...
- [ ] **Choose a retention window.** `RAMOSE_RETAIN_ROOTS` (default **20**)
...
- [ ] **Understand the unit.** Roots are published per index run, and index
      runs are driven by `RAMOSE_INDEX_TX_THRESHOLD` (500 transactions) and
      `RAMOSE_INDEX_INTERVAL_MS` (5 s). "20 roots" is therefore roughly "the
...
- [ ] **Know what a teardown does and does not do.** Deleting a
      `Ramose.Database` or `Ramose.Server` resource deletes no data, on
...
- [ ] **`limit` bounds what you receive, not what the query scans.** Sorting
      ...
      exceed `RAMOSE_QUERY_MAX_CELLS`. Narrow with `where`, not with `limit`.
...
- [ ] **Install the catalog from one place.** `Ramose.Database` at deploy for
