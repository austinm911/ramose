...
A **write** is one all-or-nothing group of changes; Ramose calls it a transaction ([glossary](/concepts/glossary/#write)). That is why a write can be built in one place and run in another, and why a failed step stops the whole thing before anything is sent.
