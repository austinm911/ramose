...
1. **Structure** — drop the hand-rolled `.rg-hero` + the `main:has(.rg-hero)` h1-hiding hack
   ...
   Frontmatter `title` carries the word "Ramose" so `<title>` and `og:title` do (F3.6).
...
3. **Try it** (fix 4) — the fake `› ramose.db(...)` strip becomes the real repo command block
...
4. **Three code moments** (fix 2) — real snippets, each fence with `title=`:
   (a) typed catalog + a write that does not compile, (b) `Ramose.query` + `db.live`,
   (c) `Ramose.Policy` rule + what a denial returns (`Unauthorized` 403 at ingress,
...
6. **Instead of what** — compact 4-row honest strip (Postgres+ORM / Convex / Supabase /
   Instant) plus a one-line list of what Ramose does not have.
...
8. **Closing CTA** — "Changes ramose." survives here only, as a small eyebrow.
