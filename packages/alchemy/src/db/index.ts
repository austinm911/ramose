/**
 * `@ramose/alchemy/db` — the portable half of Ramose.
 *
 * Schema, connecting, the database and the tagged errors, in one flat
 * namespace: `import * as Ramose from "@ramose/alchemy/db"`. It runs in a
 * browser, in a Worker, in Node/Bun and in a test.
 *
 * **Nothing reachable from this module imports `alchemy`** (the deploy engine)
 * or the `@ramose/core` barrel — that is what makes it browser-safe without a
 ...
 * transport layers, `Policy`) lives in `@ramose/alchemy`.
 *
 * ```typescript
