 ...
 * `useDb(name, catalog)` memoises a `Db` from it. On top sit the reads —
 * `useLive` (standing query as state), `useQuery` (one-shot `db.q` as
 * `Async`), `usePull` (standing `db.livePull` as `Live`), `useBasis` (where
 * the basis is) — plus `useTransact()` for writes from event handlers (works
