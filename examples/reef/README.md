...
against a compiled `Ripple.Policy`), immutability (`db.asOf` time travel +
`db.history`), and a StyleX design system with dark/light themes. The UI is
written entirely with the `@ripple/react` hooks.
...
| `src/app/ripple.ts` | the workspace wiring: `Ripple.token.jwt` over `authClient.ripple.token` plus first-entry provisioning, handed to screens as `{ slug, cls, token, myEid }` — the living client is `<RippleProvider key={slug}>`'s own |
...
- **Workspace picker** — multi-tenancy. Creating "Coral Team" runs
  ...
  `<RippleProvider key={slug}>` closes the old client and connects a new one
  whose `Ripple.token.jwt` source re-mints as the token nears `exp`, so
