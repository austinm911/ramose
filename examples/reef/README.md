...
| `src/app/ramose.ts` | the workspace wiring: `Ramose.token.jwt` over `authClient.ramose.token` plus create-time `install()` / seeds; handed to screens as `{ slug, cls, token }` — `ensureSelf` and `myEid` run on `<RamoseProvider key={slug}>`'s client |
